...
/*
~~ DEV PLAN ~~
[Y] Make sure that the texture dims are powers of 2
[Y] Make sure that the bitmaps are 24bit
[Y] Load textures
[Y] Add texture to the icos class
	[Y] Point an object at a texture
	[Y] Display a random texture on each face
[?] Make sure that GL_MODULATE is set in order to do lighting + textures - Seems to work without this
[Y] Add texture to the cylinder function
{ } Re-implement HW6: GLUT --> SDL
	{ } Implement the loop from the SDL example
